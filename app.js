...
document.getElementById('cardTable').innerHTML=`<thead><tr><th>Date / Time</th><th>Away</th><th>@</th><th>Home</th><th>Market Spread</th><th>Total</th><th>Confidence</th><th>Status</th><th>Quick Read / Why</th><th></th></tr></thead><tbody>${d.games.map(g=>`<tr><td>${g.day} ${g.time}</td><td class="teamcell"><span class="team-badge">${g.away}</span>${g.away}</td><td>@</td><td class="teamcell"><span class="team-badge">${g.home}</span>${g.home}</td><td><b>${g.market}</b></td><td>${g.total}</td><td class="conf">${g.confidence}</td><td>${badge(g.status)}</td><td class="why">${g.quick}${g.status==='WATCH'?`<div class="watch-mini"><b>Actionable:</b> ${g.actionableIf}<br><b>Kills it:</b> ${g.killsPlayIf}</div>`:''}${g.secondaryWatch?`<div class="watch-mini secondary"><b>Secondary WATCH — ${g.secondaryWatch.angle}</b><br><b>Actionable:</b> ${g.secondaryWatch.actionableIf}<br><b>Kills it:</b> ${g.secondaryWatch.killsPlayIf}</div>`:''}</td><td><a class="view" href="game.html?id=${g.id}">View</a></td></tr>`).join('')}</tbody>`;
